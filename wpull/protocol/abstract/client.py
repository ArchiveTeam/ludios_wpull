...
class BaseSession(HookableMixin, metaclass=abc.ABCMeta):
    ...
    async def _acquire_request_connection(self, request):
        '''Return a connection.'''
        ...
        connection = await self._acquire_connection(host, port, use_ssl, tunnel)

        return connection

    async def _acquire_connection(self, host, port, use_ssl=False, tunnel=True):
        '''Return a connection.'''
